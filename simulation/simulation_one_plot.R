...
d50=3# this is the range at which 50% of critters are detection
...
# loop starts here
# number of targets - in this case its targets over a set of frames, so much denser than what you would expect
reps=100
#########################################################################
...
for (i in 1:reps){
  ...
  # estimate density
  estimated_density1[i]=(length(detected_fish_ranges)/eff_vol_sim)/(n/(10^3))
}
#########################################################################
...
for (i in 1:reps){
  ...
  eff_vol_sim=integrate(eff_vol_func,lower =0,
                        upper =max(fish_range), volume_out$vol_func, detection_out$detect.function)$value
  # estimate density
  estimated_density2[i]=(length(detected_fish_ranges)/eff_vol_sim)/(n/(10^3))
}
#########################################################################
n = 1000000
...
for (i in 1:reps){
  ...
  eff_vol_sim=integrate(eff_vol_func,lower =0,
                        upper =max(fish_range), volume_out$vol_func, detection_out$detect.function)$value
  # estimate density
  estimated_density3[i]=(length(detected_fish_ranges)/eff_vol_sim)/(n/(10^3))
}
...
p1 <- ggplot(data=df, aes(group=bin, y=val)) +
  geom_boxplot() +
  labs(title = "a", x = "target density", y = "est. dens. / true dens")  +
  theme_bw() +
  scale_x_discrete(expand = c(0.1, 0),name="target density", limits=c(-0.25,0,.25),labels=c(expression("low=10/m"^"3"),expression("medium=100/m"^"3"),expression("high=1000/m"^"3"))) +
...
for (i in 1:reps){
  ...
  # run our package estimation
  prep_out=prep_detection_data(target_ranges=detected_fish_ranges,
                               vol_func=volume_out$vol_func, nbins=15, method='max', nvals=3, loc_dens=NULL, plotting=FALSE)
...
for (i in 1:reps){
  ...
  # run our package estimation
  prep_out=prep_detection_data(target_ranges=detected_fish_ranges,
                               vol_func=volume_out$vol_func, nbins=15, method='mean', nvals=5, loc_dens=NULL, plotting=FALSE)
...
p2 <- ggplot(data=df, aes(group=bin, y=val)) +
  geom_boxplot()+
  labs(title = "b", x = "peak detection method", y = "est. dens. / true dens")  +
  theme_bw() +
  scale_x_discrete(expand = c(0.1, 0),name="peak detection method", limits=c(-0.25,0,.25),labels=c("max","mean (n=3)","mean (n=5)")) +
  geom_hline(yintercept=1)
...
colnames(df) <- c("bin","val")
df$val=df$val/100
p3 <- ggplot(data=df, aes(group=bin, y=val)) +
  geom_boxplot()+
  labs(title = "c", x = "bin size", y = "est. dens. / true dens")  +
 theme_bw() +
  scale_x_discrete(expand = c(0.1, 0),name="bin size", limits=c(-0.25,0,.25),labels=c("low=10","medium=15","high=20")) + geom_hline(yintercept=1)
#print(p1)
...
colnames(df) <- c("bin","val")
df$val=df$val/100
p4 <- ggplot(data=df, aes(group=bin, y=val)) +
  geom_boxplot()+
  labs(title = "d", x = "bin size", y = "est. dens. / true dens")  +
  theme_bw() +
  scale_x_discrete(expand = c(0.1, 0),name="detection distance",
