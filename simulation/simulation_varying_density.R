...
p1 <- ggplot(data=df, aes(group=bin, y=val)) +
  geom_boxplot()+labs(title = "", x = "target density", y = "est. dens. / true dens")  +
 theme_bw() +scale_x_discrete(expand = c(0.1, 0),name="target density", limits=c(-0.25,0,.25),labels=c(bquote("low=10/m^3"),bquote("medium=100/m^3"),bquote("high=1000/m^3")))
