...
d50=5# this is the range at which 50% of criters are detection
...
for (i in 1:reps){
  ...
  detection_data=prep_detection_data(target_ranges=detected_fish_ranges,
                                     vol_func=vol$vol_func, nbins=NULL, method='mean', nvals=3, loc_dens=NULL, plotting=FALSE)
  ...
  # estimate density
  estimated_density[i]=length(detected_fish_ranges)/eff_vol_sim
}
