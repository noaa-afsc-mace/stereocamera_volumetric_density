...
#  theme_bw() +scale_x_discrete(expand = c(0.1, 0),name="bin size", limits=c(-0.25,0,.25),labels=c("low=10","medium=15","high=20")) + geom_hline(yintercept=1)
# print(p1)
...
for (i in 1:reps){
  ...
  # run our package estimation
  prep_out=prep_detection_data(target_ranges=detected_fish_ranges,
                               vol_func=volume_out$vol_func, nbins=NA, method='mean', nvals=3, loc_dens=NULL, plotting=FALSE)
