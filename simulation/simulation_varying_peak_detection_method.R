...
for (i in 1:reps){
  ...
  # run our package estimation
  prep_out=prep_detection_data(target_ranges=detected_fish_ranges,
                                     vol_func=volume_out$vol_func, nbins=15, method='mean', nvals=2, loc_dens=NULL, plotting=FALSE)
...
p1 <- ggplot(data=df, aes(group=bin, y=val)) +
  geom_boxplot()+labs(title = "", x = "peak detection method", y = "est. dens. / true dens")  +
 theme_bw() +scale_x_discrete(expand = c(0.1, 0),name="peak detection method", limits=c(-0.25,0,.25),labels=c("max","mean (n=2)","mean (n=3)")) + geom_hline(yintercept=1)
print(p1)
