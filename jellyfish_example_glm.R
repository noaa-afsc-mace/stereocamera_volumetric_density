...
out1 <- fit_single_logistic(target_ranges=jellyfish$targets$RANGE,vol_func=vol_func$vol_func,nbins=20,plotting=FALSE,start_params = c(1500,1,-1))
# make expected density equal to the "scale" parameter from the optim curve fit model
#exp_dens=out1$model_output$par[1]
# make expected density equal to median of five highest values
dens_sort=sort(out1$density_data$dens,decreasing=TRUE)
exp_dens=median(dens_sort[1:5])
out2 <- fit_single_logistic_glm(target_ranges=jellyfish$targets$RANGE,vol_func=vol_func$vol_func,nbins=20,plotting=FALSE, exp_dens=exp_dens)


exp_count=exp_dens*out1$density_data$vol
prop_detect=out1$density_data$bin_count/exp_count
plot(out1$density_data$rng,prop_detect,xlab='range from camera (m)',ylab='observed/expected')
prop_detect_true=prop_detect
ind=which(prop_detect_true>1)
prop_detect_true[ind]=1
points(out1$density_data$rng[ind],prop_detect_true[ind],pch=19)
